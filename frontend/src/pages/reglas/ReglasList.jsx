import { useEffect, useState } from "react";
import api from "../../services/api";
import { endpoints } from "../../services/endpoints";
import Modal from "../../components/Modal";
import ReglaForm from "./ReglaForm";

export default function ReglasList() {
  const [reglas, setReglas] = useState([]);
  const [open, setOpen] = useState(false);
  const [editing, setEditing] = useState(null);

  // Cargar reglas al inicio
  const loadReglas = () => {
    api.get(endpoints.reglas)
      .then((res) => setReglas(res.data))
      .catch((err) => console.error(err));
  };

  useEffect(() => {
    loadReglas();
  }, []);

  // Eliminar
  const deleteRegla = (id) => {
    if (!confirm("¿Seguro que desea eliminar esta regla?")) return;

    api.delete(`${endpoints.reglas}${id}/`)
      .then(() => loadReglas())
      .catch((err) => console.error(err));
  };

  return (
    <div className="p-6">
      <div className="flex justify-between mb-6">
        <h2 className="text-2xl font-bold text-primary">Reglas de Puntos</h2>

        <button
          className="bg-primary text-white px-4 py-2 rounded"
          onClick={() => { setEditing(null); setOpen(true); }}
        >
          + Nueva Regla
        </button>
      </div>

      {/* TABLA */}
      <table className="min-w-full bg-white shadow rounded">
        <thead>
          <tr className="bg-secondary text-white">
            <th className="py-2 px-4">ID</th>
            <th className="py-2 px-4">Límite Inferior</th>
            <th className="py-2 px-4">Límite Superior</th>
            <th className="py-2 px-4">Monto por Punto</th>
            <th className="py-2 px-4">Acciones</th>
          </tr>
        </thead>

        <tbody>
          {reglas.map((r) => (
            <tr key={r.id} className="border-b hover:bg-gray-50">
              <td className="py-2 px-4">{r.id}</td>
              <td className="py-2 px-4">{r.limite_inferior}</td>
              <td className="py-2 px-4">{r.limite_superior ?? "Sin límite"}</td>
              <td className="py-2 px-4">{r.monto_por_punto}</td>

              <td className="py-2 px-4 space-x-3">
                <button
                  className="text-blue-600"
                  onClick={() => { setEditing(r); setOpen(true); }}
                >
                  Editar
                </button>

                <button
                  className="text-red-600"
                  onClick={() => deleteRegla(r.id)}
                >
                  Eliminar
                </button>
              </td>
            </tr>
          ))}
        </tbody>
      </table>

      {/* Modal */}
      <Modal open={open} onClose={() => setOpen(false)}>
        <ReglaForm
          regla={editing}
          onSuccess={() => { setOpen(false); loadReglas(); }}
        />
      </Modal>
    </div>
  );
}

import { useEffect, useState } from "react";
import api from "../../services/api";
import { endpoints } from "../../services/endpoints";
import Modal from "../../components/Modal";
import ConceptoForm from "./ConceptoForm";

export default function ConceptosList() {
  const [conceptos, setConceptos] = useState([]);
  const [open, setOpen] = useState(false);
  const [editing, setEditing] = useState(null);

  // Cargar conceptos
  const loadConceptos = () => {
    api.get(endpoints.conceptos)
      .then((res) => setConceptos(res.data))
      .catch((err) => console.error(err));
  };

  useEffect(() => {
    loadConceptos();
  }, []);

  // Eliminar concepto
  const deleteConcepto = (id) => {
    if (!confirm("¿Seguro que desea eliminar este concepto?")) return;

    api.delete(`${endpoints.conceptos}${id}/`)
      .then(() => loadConceptos())
      .catch((err) => console.error(err));
  };

  return (
    <div className="p-6">
      <div className="flex justify-between mb-6">
        <h2 className="text-2xl font-bold text-primary">Conceptos de Puntos</h2>
        <button
          className="bg-primary text-white px-4 py-2 rounded"
          onClick={() => { setEditing(null); setOpen(true); }}
        >
          + Nuevo Concepto
        </button>
      </div>

      {/* TABLA */}
      <table className="min-w-full bg-white shadow rounded">
        <thead>
          <tr className="bg-secondary text-white">
            <th className="py-2 px-4">ID</th>
            <th className="py-2 px-4">Descripción</th>
            <th className="py-2 px-4">Puntos Requeridos</th>
            <th className="py-2 px-4">Acciones</th>
          </tr>
        </thead>

        <tbody>
          {conceptos.map((c) => (
            <tr key={c.id} className="border-b hover:bg-gray-50">
              <td className="py-2 px-4">{c.id}</td>
              <td className="py-2 px-4">{c.descripcion}</td>
              <td className="py-2 px-4">{c.puntos_requeridos}</td>

              <td className="py-2 px-4 space-x-3">
                <button
                  className="text-blue-600"
                  onClick={() => { setEditing(c); setOpen(true); }}
                >
                  Editar
                </button>

                <button
                  className="text-red-600"
                  onClick={() => deleteConcepto(c.id)}
                >
                  Eliminar
                </button>
              </td>
            </tr>
          ))}
        </tbody>
      </table>

      {/* MODAL */}
      <Modal open={open} onClose={() => setOpen(false)}>
        <ConceptoForm
          concepto={editing}
          onSuccess={() => { setOpen(false); loadConceptos(); }}
        />
      </Modal>
    </div>
  );
}
